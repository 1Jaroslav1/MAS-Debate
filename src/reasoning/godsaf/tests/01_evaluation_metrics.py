...
# Assuming ASPSolver is in the correct path relative to this script
# or installed in the environment.
from src.reasoning.asp.solver import ASPSolver # Zakładamy, że ten import działa
...
def run_test_eval_proponent_key_argument_impact():
    ...
    assert "scope(arg_pro_main,economic_effects)" in facts
    assert "scope(arg_pro_main,social_impact)" in facts
    
    # Verify APC calculations
    assert any("apc(arg_pro_main,reduce_poverty,economic_effects," in f for f in facts)
    assert any("apc(arg_pro_main,improve_public_health,social_impact," in f for f in facts)
    
    # Verify challenger count
    assert "num_effective_opponent_challengers(arg_pro_main,economic_effects,1)" in facts
    assert "num_effective_opponent_challengers(arg_pro_main,social_impact,0)" in facts
    
    # Verify EAPC calculations
    assert any("eapc(arg_pro_main,reduce_poverty,economic_effects," in f for f in facts)
    assert any("eapc(arg_pro_main,improve_public_health,social_impact," in f for f in facts)
    
    # Verify APS calculation
    assert any("aps(arg_pro_main," in f for f in facts)
    
    # Verify TGS calculation
    assert any("tgs(team_pro_ubi,reduce_poverty,economic_effects," in f for f in facts)
